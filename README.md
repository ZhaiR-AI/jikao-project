...
在自己的 Windows 电脑上运行：导入 PDF、Word（.docx）、TXT 或 Markdown 试卷，生成可作答的试题、判卷并管理错题和学习记录。

本仓库只提供程序，不附带作者的卷子、学习资料、API Key 或个人记录。首次启动时试卷列表为空，需要导入自己的试卷文件。

## 普通用户：下载 Windows 便携版

**不懂编程、没有安装 Python？请使用便携版。**

> **PDF 模型要求：必须使用支持图片输入和文字识别的多模态大模型（视觉模型）。** PDF 一律逐页转成图片识别，不使用内嵌文字层绕过识别。纯文字聊天模型、只能生成图片的模型不适用于 PDF；请确认模型平台的 API 也支持图片输入。

DOCX、TXT、Markdown 会先读取文字，再交给大模型整理。Word 中的图片、公式对象和复杂自动编号请先导出为 PDF，以免漏掉内容；旧版 `.doc` 请另存为 `.docx` 或 PDF。文字文档按“内容段”定位原文，PDF 按页定位。听力原卷若只有选项，需要自行播放配套音频，软件不会编造音频中的题干。
